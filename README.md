...
| **Head hiding** | Renders the head shadows-only: it vanishes from view but keeps animating and still casts its shadow, and nothing is written into your character or scene files | Flips the character's stored "head always visible" flag (optional, off by default) | Deactivates the whole head object; since Unity stops animating deactivated bones, the eye position can freeze while the body keeps moving, letting the camera drift and clip into the body |
